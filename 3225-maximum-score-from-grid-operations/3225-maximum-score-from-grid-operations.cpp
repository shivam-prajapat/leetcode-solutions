...
public:
    long long maximumScore(vector<vector<int>>& grid) {
        ...
        vector<long long> prevPick(n + 1, -1e15);
        vector<long long> prevSkip(n + 1, -1e15);
        prevPick[0] = 0;
        prevSkip[0] = 0;

        for (int j = 0; j < n; ++j) {
            vector<long long> currPick(n + 1, -1e15);
            vector<long long> currSkip(n + 1, -1e15);
            
            for (int curr = 0; curr <= n; ++curr) {
                for (int prev = 0; prev <= n; ++prev) {
                    if (curr > prev) {
                        long long score = (j > 0) ? prefix[j - 1][curr] - prefix[j - 1][prev] : 0;
